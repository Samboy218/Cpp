...
class compound:
    ...
    def print(self):
        ...
        for atom in self.atoms:
            curr_atom_ind = atom_ind
            string = string + 'v {} atom\n'.format(curr_atom_ind)
            atom.node_id = curr_atom_ind
            atom_ind += 1
            string += 'v {} {}\n'.format(atom_ind, atom.element)
            string += 'u {} {} element\n'.format(curr_atom_ind, atom_ind)
            atom_ind += 1
            string += 'v {} {}\n'.format(atom_ind, atom.type)
            string += 'u {} {} atom_type\n'.format(curr_atom_ind, atom_ind)
            atom_ind += 1
        #now do the bonds
        for bond in self.bonds:
            #now the bonds actually have nodes
            curr_ind = atom_ind
            string += 'v {} bond\n'.format(curr_ind)
            atom_ind += 1
            string += 'v {} {}\n'.format(atom_ind, bond.type)
            string += 'u {} {} bond_type\n'.format(curr_ind, atom_ind)
            atom_ind += 1
            atom1 = bond.atoms[0]
            atom2 = bond.atoms[1]
            atom1_node = -1
            ...
            string = string + 'u {} {} connect\n'.format(curr_ind, atom1_node)
            string = string + 'u {} {} connect\n'.format(curr_ind, atom2_node)
        return string
...
with open('atom_bond_subdue2.g', 'w') as f:
    f.write(str_file)
